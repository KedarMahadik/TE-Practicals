...
def submit_registration():
    ...
    if not patient_name or not patient_age or not patient_email or not patient_address or not emergency_contact:
...
tk.Label(main_frame, text="Address:", font=("Helvetica", 12), fg="#34495E").grid(row=5, column=0, sticky="ne", pady=5)
...
tk.Label(main_frame, text="Blood Group:", font=("Helvetica", 12), fg="#34495E").grid(row=6, column=0, sticky="e", pady=5)
blood_group_var = tk.StringVar()
tk.OptionMenu(main_frame, blood_group_var, "A+", "B+", "O+", "AB+", "A-", "B-", "O-", "AB-").grid(row=6, column=1, pady=5)
...
tk.Button(main_frame, text="Submit", command=submit_registration, font=("Helvetica", 14, "bold"), bg="#2ecc71", fg="white").grid(row=9, column=0, columnspan=2, pady=20)
tk.Button(main_frame, text="Clear", command=clear_form, font=("Helvetica", 12), bg="#f39c12", fg="white").grid(row=10, column=0, columnspan=2, pady=5)
